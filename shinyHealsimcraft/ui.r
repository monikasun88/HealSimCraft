...
shinyUI(pageWithSidebar(
  ...
  sidebarPanel(
    h4("Enter stats here:"),
    helpText("Please enter stat values from items only."),
    ...
    numericInput("mastery_I", "Mastery (points):", 0),
    numericInput("criticalstrike_I", "Critical Strike (points):", 0),
    numericInput("sp_I", "Spell Power(weapon):", 0),
    numericInput("haste_I", "Haste (points):", 0),    
    numericInput("dodge_I", "Dodge (points):", 0),
    numericInput("parry_I", "Parry (points):", 0),
    numericInput("expertise_I", "Expertise (points):", 0),
    numericInput("hit_I", "Hit (points):", 0),
    
    selectInput("race", "Choose your race (only for non-weapon specific stats):", 
                choices = c("Tauren","Troll","Blood Elf","Pandaren","Undead","Goblin","Orc",
                            "Worgen","Draenei","Night Elf","Gnome","Dwarf","Human")), 
